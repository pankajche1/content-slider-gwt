...
public class ContentSlider1Animation extends Animation {
	private ContentSlider1 view;
	...
	public ContentSlider1Animation(ContentSlider1 view,ItemsContainer target) {
		this.view=view;
		this.target = target;
			...
			if(isSwitchDirection){
				...
				//setting these three parameters;
					...
					if(scrollMode==RIGHT_TO_LEFT){//left <- right
						...
						//before the rest it was scrolling right to left
						target.shiftLastToFirst();
						elem.getStyle().setLeft(-220, Style.Unit.PX);//for left to right scroll
						stateMode=LEFT_RIGHT_STATE;
					}
